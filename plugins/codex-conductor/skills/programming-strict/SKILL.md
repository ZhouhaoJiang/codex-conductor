---
name: programming-strict
description: "Explicit opt-in strict programming reference for Python, Rust, TypeScript, and Go. Use only when the user names programming-strict or asks for the full strict toolchain and TDD discipline; never auto-activate for ordinary code edits."
---

# Programming

> Modified from the OMO 4.17.1 shared `programming` skill: Conductor renames it
> and makes it explicit opt-in. See `LICENSE.md`.

Only activate this skill when the user explicitly asks for `programming-strict`
or for this full strict engineering profile. Repository instructions and the
user's requested stack outrank the defaults below.
